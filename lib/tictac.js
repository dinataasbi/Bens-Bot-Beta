...
Giliran @${posi.penantang.split("@")[0]}`,
        ...
        if (sender === posi.penantang) {
          for (let i of nomor) {
            if (Number(chats) === i) {
              if (cekIsi(Number(chats) - 1, anu))
                return reply(`Nomor tersebut sudah terisi`);
              tictactoe[getPosTic(from, tictactoe)].TicTacToe[
                Number(chats) - 1
              ] = "❌";
