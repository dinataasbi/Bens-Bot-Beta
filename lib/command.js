class Command {
  ...
  on(eventName, command = [], usePrefix = true, callback = () => {}, opt = {}) {
    ...
    let nitip = !this._events[eventName]
      ? `Register Event|Command ${eventName}`
      : `Checking Event|Command ${eventName}`;
    this._events[eventName] = {
      eventName,
      command: new RegExp(`^${command.join("|")}`),
      usePrefix,
      callback,
      ...opt,
      enable: true,
  ...
  exec(msg) {
    return new Promise(async (resolve, reject) => {
      for (let eventCmd in this._events) {
        ...
        let prefix = !event.usedPrefix
          ? msg.command.exec(msg.StringMsg)
          : this.prefix.exec(msg.StringMsg) ||
            this.default_prefix.exec(msg.StringMsg);
        if (!prefix) return;
        let total_args = msg.StringMsg.split(" ");
        let args = total_args.slice(1);
        let usedPref = prefix[0];
        let command = !event.usedPrefix
          ? usedPref
          : msg.StringMsg.replace(usedPref, "");
        let commandNpref = !this._events[eventCmd].usedPrefix
          ? command
          : usedPref + command;
        if (!command) return;
