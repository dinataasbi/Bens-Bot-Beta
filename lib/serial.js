...
exports.jClient = async function ({ tabrak, store }) {
  ...
  const client = Object.defineProperties(tabrak, {
    copyNForward: {
      async value(jid, message, forceForward = false, options = {}) {
        ...
        await tabrak.relayMessage(jid, waMessage.message, {
...
exports.jSerialize = (tabrak, msg) => {
  ...
  if (msg.key) {
    ...
    m.sender = m.fromMe ? tabrak.user.id : m.isGroup ? m.participant : m.from;
  }

  m.pushName = msg.pushName;
  if (m.isGroup) {
    m.metadata = tabrak.groupMetadata(m.from);
  }
  ...
  if (m.message) {
    ...
    m.reply = (teks) => {
      tabrak.sendMessage(m.from, { text: teks }, { quoted: m });
    };

    m.copyNForward = (jid = m.chat, forceForward = false, options = {}) =>
      tabrak.copyNForward(jid, m, forceForward, options);
  }
  ...
  if (m.msg?.contextInfo?.quotedMessage) {
    ...
    if (m.quoted.message) {
      ...
      m.quotedMsg.fromMe =
        m.quotedMsg.sender === tabrak.user.id.split(":")[0] + "@s.whatsapp.net"
          ? true
          : false;
      ...
      if (m.quoted.isGroup) {
        m.quoted.metadata = tabrak.groupMetadata(m.quoted.from);
      }
      ...
      m.quoted.download = (filepath) => {
        if (filepath) return tabrak.downloadMediaMessage(m.quoted, filepath);
        else return tabrak.downloadMediaMessage(m.quoted);
      };
    }
  }
